...
@app.route('/ProductVeggies', methods = ['GET', 'POST'])
def product_veggies() :
    pysql.init()

    # Get all the veggies
    product_details = Product.get_product_by_category(pysql, 'Veggies')
    global all_ids
            ...
            for i in range(0, len(product_details)) :
                if quantities[i] > 0 :
                    if not(Product.check_if_in_stock(pysql, all_ids['customer_id'], product_details[i][0], quantities[i])) :
                        ...
                        return render_template('/Product/product_veggies.html', product_details = product_details)
...
@app.route('/ProductDairy', methods = ['GET', 'POST'])
def product_dairy() :
    pysql.init()
    product_details = Product.get_product_by_category(pysql, 'Dairy')
    global all_ids
    ...
    if request.method == 'POST' :
        ...
        if 'buy_now' in request.form :
            ...
            if (Cart.get_no_of_products_in_cart(pysql, all_ids['customer_id']) + sum(quantities)) > 5 :
                print("Max cart limit reached")
                return render_template('/Product/product_dairy.html', product_details = product_details)
...
@app.route('/ProductBakery', methods = ['GET', 'POST'])
def product_bakery() :
    pysql.init()
    product_details = Product.get_product_by_category(pysql, 'Bakery')
    global all_ids
    ...
    if request.method == 'POST' :
        ...
        if 'buy_now' in request.form :
            ...
            if (Cart.get_no_of_products_in_cart(pysql, all_ids['customer_id']) + sum(quantities)) > 5 :
                print("Max cart limit reached")
                return render_template('/Product/product_bakery.html', product_details = product_details)
...
@app.route('/ProductFoodgrains', methods = ['GET', 'POST'])
def product_foodgrains() :
    pysql.init()
    product_details = Product.get_product_by_category(pysql, 'Foodgrains')
    global all_ids
    ...
    if request.method == 'POST' :
        ...
        if 'buy_now' in request.form :
            ...
            if (Cart.get_no_of_products_in_cart(pysql, all_ids['customer_id']) + sum(quantities)) > 5 :
                print("Max cart limit reached")
                return render_template('/Product/product_foodgrains.html', product_details = product_details)
...
@app.route('/ProductCleaning', methods = ['GET', 'POST'])
def product_cleaning() :
    pysql.init()
    product_details = Product.get_product_by_category(pysql, 'Cleaning')
    global all_ids
    ...
    if request.method == 'POST' :
        ...
        if 'buy_now' in request.form :
            ...
            if (Cart.get_no_of_products_in_cart(pysql, all_ids['customer_id']) + sum(quantities)) > 5 :
                print("Max cart limit reached")
                return render_template('/Product/product_cleaning.html', product_details = product_details)
